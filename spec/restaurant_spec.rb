...
describe Restaurant do
  ...
  describe 'attributes' do
    ...
    it "allow reading and writing for :name" do
      crescent.name = "Dukes House of Chicken and Waffels"
      expect(crescent.name).to eq("Dukes House of Chicken and Waffels")
    end

    it "allow reading and writing for :cuisine" do
      crescent.cuisine = "Chicken and Waffels"
      expect(crescent.cuisine).to eq("Chicken and Waffels")
    end

    it "allow reading and writing for :price" do
      crescent.price = "Very expensive"
      expect(crescent.price).to eq("Very expensive")
    end
  ...
  describe '.all' do
    ...
    it 'returns an empty array when @@file is nil' do
      ...
      expect(restaurants.length).to eq(0)
    end
  ...
  describe '#initialize' do
    ...
    context 'with custom options' do
      ...
      it 'allows setting the :name' do
        restaurant = Restaurant.new({name: "Dukes House of Chicken and Waffels"})
        expect(restaurant.name).to eq("Dukes House of Chicken and Waffels")
      end

      it 'allows setting the :cuisine' do
        restaurant = Restaurant.new({cuisine: "Chicken and Waffels"})
        expect(restaurant.cuisine).to eq("Chicken and Waffels")
      end

      it 'allows setting the :price' do
        restaurant = Restaurant.new({price: "Very expensive"})
        expect(restaurant.price).to eq("Very expensive")
      end
  ...
  describe '#save' do
    ...
    it 'returns false if @@file is nil' do
      restaurant = Restaurant.new({ price: 500, name: "The place" })
